...
void Sample3DSceneRenderer::CreateDeviceDependentResources()
{
	...
	auto createAssetsTask = createPipelineStateTask.then([this]() {
		...
		{
			{ XMFLOAT3(-1.0f, 3.0f, 0.0f), XMFLOAT2(-1.0f, 3.0f) },
			{ XMFLOAT3(3.0f, -1.0f,  0.0f), XMFLOAT2(3.0f, -1.0f) },
		...
		m_vertexBufferView.StrideInBytes = sizeof(VertexPositionUV);
...
void Sample3DSceneRenderer::CreateWindowSizeDependentResources()
{
	...
	XMVECTOR screenSize = { (float)outputSize.Width, (float)outputSize.Height };

	XMStoreFloat4x4(&m_constantBufferData.view, XMMatrixTranspose(XMMatrixLookAtRH(eye, at, up)));

	XMStoreFloat2(&m_constantBufferData.screenSize, screenSize);

}

bool g_WasResized = false;
...
void Sample3DSceneRenderer::Update(DX::StepTimer const& timer)
{
	if (m_loadingComplete)
	{
		if (!m_tracking)
		{
			...
			m_angle += static_cast<float>(time) * m_radiansPerSecond;

			Rotate(m_angle);

			float fTime = static_cast<float>(timer.GetTotalSeconds());
			XMVECTOR tv = { fTime };
			XMStoreFloat(&m_constantBufferData.iTime, tv);
